...
public class XiaochanHttp {
    ...
    /**
     * 获取小蚕美团赏金数据（本地实例方法，走 executeWithProxy 代理）。
     * 上游为静态直连，本地统一改走 {@link #postWithRes}（内部 executeWithProxy + ProxyHolder.SHARED_KEY）。
     * @param longitude 经度
     * @param latitude  纬度
     * @param pvId      上一页的 pv_id
     * @return 美团赏金门店列表
     */
    public List<StoreInfo> getMeituanList(String longitude, String latitude, String pvId){
        ...
        body.put("scene", 1);
        body.put("app_id", 20);
        // cityCode 传 null：美团赏金列表/搜索无登录态，与本地其它列表共用 shared 出口
        String resBody = postWithRes(BASE_URL, JSONObject.toJSONString(body), null,
                "SilkwormFusion", "FusionService.GetMeiTuanPromotions");
        JSONObject jsonObject = checkResult(resBody);
        return parseMeituanListBody(jsonObject.getJSONArray("list"));
    }

    /**
     * 搜索小蚕美团赏金数据（本地实例方法，走 executeWithProxy 代理）。
     * @param longitude 经度
     * @param latitude  纬度
     * @param name      搜索关键词
     * @param pvId      上一页的 pv_id
     * @return 美团赏金门店列表
     */
    public List<StoreInfo> searchMeituanList(String longitude, String latitude, String name, String pvId){
        Map<String, Object> body = new HashMap<>();
        body.put("lat", new BigDecimal(latitude));
        ...
        body.put("search_word", name);
        body.put("app_id", 20);
        String resBody = postWithRes(BASE_URL, JSONObject.toJSONString(body), null,
                "SilkwormRcs", "SilkwormRcsService.MeituanShangjinGetPoiList");
        JSONObject jsonObject = checkResult(resBody);
        return parseMeituanListBody(jsonObject.getJSONArray("poi_list"));
    }

    /**
     * 解析小蚕美团赏金 body 中的 poi 数组。
     * 每 poi 有若干 plan_activity_info_list，每个活动产出一个 StoreInfo（按活动拆行）。
     */
    private List<StoreInfo> parseMeituanListBody(JSONArray poiList) {
        if (poiList == null) {
        ...
        for (int i = 0; i < poiList.size(); i++) {
            ...
            // 默认门店类型置为小蚕美团赏金（无活动行时保底）
            storeInfo.setStoreTypeEnum(StoreTypeEnum.XC_MTSJ);
            ...
            if (plans == null) {
                ...
                for (int j = 0; j < plans.size(); j++) {
                    ...
                    // 本地 promotionId 为 Integer，上游赋 String，适配为 Integer.parseInt
                    // （poi_event_id 为美团活动 id，可安全解析；解析失败回退 null 不致崩溃）
                    String poiEventId = activity.getString("poi_event_id");
                    if (poiEventId != null) {
                        try {
                            item.setPromotionId(Integer.parseInt(poiEventId));
                        } catch (NumberFormatException e) {
                            log.warn("parseMeituanListBody poi_event_id 非数字(={}), promotionId 置 null", poiEventId);
                        }
                    }
